...
export class shareOnMastodon extends HTMLElement {
  constructor() {
    ...
    if (this.getAttribute("button_text")) {
      this.shadowRoot.querySelector("#btnText").innerHTML =
        this.getAttribute("button_text");
    }

    // Get the modal heading from atrribute if available
    if (this.getAttribute("modal_heading")) {
      this.shadowRoot.querySelector("#modalHeading").innerHTML =
        this.getAttribute("modal_heading");
    }

    // Get the modal text from atrribute if available
    if (this.getAttribute("modal_text")) {
      this.shadowRoot.querySelector("#modalText").innerHTML =
        this.getAttribute("modal_text");
    }

    // Get the default url to use as placeholder text from atrribute if available
    if (this.getAttribute("default_url")) {
      this.shadowRoot.querySelector("#instanceInput").placeholder =
        this.getAttribute("default_url");
    }

    // Get the share button text from atrribute if available
    if (this.getAttribute("share_text")) {
      this.shadowRoot.querySelector("#shareText").innerHTML =
        this.getAttribute("share_text");
    }
  }
  ...
  openModal() {
    ...
    if (localStorage["mastodon_instance"]) {
      this.shadowRoot.querySelector("#instanceInput").value =
        localStorage["mastodon_instance"];
    }
  ...
  connectedCallback() {
    this.shadowRoot
      .querySelector("#mastodonBtn")
      .addEventListener("click", () => this.openModal());
    this.shadowRoot
      .querySelector("#shareBtn")
      .addEventListener("click", () => this.validate_before_share());
  }
